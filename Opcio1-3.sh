...
echo "*************** $cataleg: El Nostre catàleg per valoració es: ********************
*****************************************************************************"
linies=wc -l < $1
for i in seq 1 $linies  
do
pelicula=cat $1 | tail +2 | sort -k5 -t, -n -r | head -$i | tail -1
...
iant=$(($i-1))
peliculanterior=cat $1 | tail +2 | sort -k5 -t, -n -r | head -$iant | tail -1 
